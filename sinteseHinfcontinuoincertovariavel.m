...
beta=-2;                     %valor inicial para o parametro beta que será otimizado
...
for n = 1:length(alfa)
    ...
    while a>0
        ...
        for l=1:N
            for i=1:N
                J = [F1+F1' P{l}-An{i}*F1-B{i}*W+alfa(n)*F1' zeros(nx,q) -Bw{i};
                    (P{l}-An{i}*F1-B{i}*W+alfa(n)*F1')' alfa(n)*(-An{i}*F1-B{i}*W-F1'*An{i}'-W'*B{i}') -F1*C{i}'*beta -alfa(n)*Bw{i};
                ...
                LMIs = LMIs+(LMI1<0)+(P{l}>0)+(abs(gama)<=1);
    ...
    while a>0
        ...
        for l=1:N
            for i=1:N
                J2 = [F1+F1' P{l}-An{i}*F1-B{i}*W+alfa(n)*F1' zeros(nx,q) -Bw{i};
                    (P{l}-An{i}*F1-B{i}*W+alfa(n)*F1')' alfa(n)*(-An{i}*F1-B{i}*W-F1'*An{i}'-W'*B{i}') -F1*C{i}'*G1 -alfa(n)*Bw{i};
                ...
                LMI2= [J2 M' N1';
                    M -gama*diag(ones(nx,1)) zeros(nx,nx);
                    N1 zeros(nx,nx) -gama*diag(ones(nx,1))];
                LMIs2 = LMIs2+(LMI2<0)+(P{l}>0)+(abs(gama)<=1);
            end
        end
        % solver
        options = sdpsettings('savesolveroutput',1,'verbose',1,'warning',1,'solver','sedumi','showprogress',0);
